...
export class FormComponent implements OnInit {
  ...
  constructor(
    formBuilder: FormBuilder, private openId: OpenidService,
  ...
  addEvent(type: string, event: MatDatepickerInputEvent<Date>) {
    if (type === "start") {
      ...
      console.log(
        `${type}: ${event.value} -> ${this.form.get("request_start_date").value}`
      );
      console.log(` ${this.form.get("request_report_date").value}`);
    } else if (type === "report") {
      this.form.get("request_report_date").setValue(event.value);
      console.log(
        `${type}: ${event.value} -> ${this.form.get("request_report_date").value}`
      );
    }
  }
  ...
  onSubmit() {
    ...
    this.openId.makeAholidayRequest(this.form.value).subscribe(date => console.log(date))
     this.msgShow = true;
  }
}
